...
var PruneClusterForLeaflet = (L.Layer ? L.Layer : L.Class).extend({
    ...
    BuildLeafletClusterIcon: function (cluster) {
        ...
        var n = cluster.population;
        if (!this.Cluster.showExactCount) {
            n = n >= 1e4 ? Math.round(n / 1e3) + "k" : n >= 1e3 ? Math.round(n / 100) / 10 + "k" : n;
            //console.log(cluster.population + " => " + n)
        }
